...
namespace http {
    ...
    static saturn::ConfigVar<uint64_t>::ptr g_http_request_buffer_size =
        saturn::Config::add("http.request.buffer_size"
    ...
    void on_request_method(void *data, const char *at, size_t length) {
        ...
        if(m == HttpMethod::INVALID_METHOD) {
            SATURN_LOG_WARN(g_logger) << "invalid http request method: "
    ...
    void on_request_version(void *data, const char *at, size_t length) {
        ...
        if(strncmp(at, "HTTP/1.1", length) == 0) {
            ...
            SATURN_LOG_WARN(g_logger) << "invalid http request version: "
                << std::string(at, length);
    ...
    void on_request_http_field(void *data, const char *field, size_t flen
        ...
        HttpRequestParser* parser = static_cast<HttpRequestParser*>(data);
        if(flen == 0) {
            SATURN_LOG_WARN(g_logger) << "invalid http request field length == 0";
    ...
    HttpRequestParser::HttpRequestParser()
        :m_error(0) {
        m_data.reset(new saturn::http::HttpRequest);
    ...
    void on_response_version(void *data, const char *at, size_t length) {
        ...
        if(strncmp(at, "HTTP/1.1", length) == 0) {
            ...
            SATURN_LOG_WARN(g_logger) << "invalid http response version: "
                << std::string(at, length);
            parser->setError(1001);
            return;
        }
    ...
    HttpResponseParser::HttpResponseParser()
        :m_error(0) {
        m_data.reset(new saturn::http::HttpResponse);
