...
HttpConnection::~HttpConnection() {
    SATURN_LOG_DEBUG(g_logger) << "HttpConnection::~HttpConnection";
}
...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    if(client_parser.chunked) {
        ...
        do {
            ...
            SATURN_LOG_INFO(g_logger) << "content_len=" << client_parser.content_len;
...
HttpConnection::ptr HttpConnectionPool::getConnection() {
    uint64_t now_ms = getCurrentTime<std::chrono::milliseconds>();
    ...
    if(!ptr) {
        ...
        if(!addr) {
            SATURN_LOG_ERROR(g_logger) << "get addr fail: " << m_host;
            return nullptr;
        }
        addr->setPort(m_port);
        Socket::ptr sock = Socket::CreateTCP(addr);
        if(!sock) {
            SATURN_LOG_ERROR(g_logger) << "create sock fail: " << *addr;
            return nullptr;
        }
        if(!sock->connect(addr)) {
            SATURN_LOG_ERROR(g_logger) << "sock connect fail: " << *addr;
            return nullptr;
        }
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected()
            || ((ptr->m_createTime + pool->m_maxAliveTime) >= getCurrentTime<std::chrono::milliseconds>())
            || (ptr->m_request >= pool->m_maxRequest)) {
        delete ptr;
        --pool->m_total;
        return;
    }
    LOCK(lock_guard, pool->m_mutex);
