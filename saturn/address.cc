...
namespace saturn {
    ...
    bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host, 
            int family, int type, int protocol) {
                ...
                if(error) {
                    SATURN_LOG_DEBUG(g_logger) << "Address::Lookup getaddress(" << host << ", "
    ...
    bool Address::GetInterfaceAddresses(std::multimap<std::string, std::pair<Address::ptr, uint32_t>>& result
            , int family) {
                ...
                if(getifaddrs(&results) != 0) {
                    SATURN_LOG_DEBUG(g_logger) << "Address::GetInterfaceAddresses getifaddrs "
                ...
                try {
                    ...
                    SATURN_LOG_ERROR(g_logger) << "Address::GetInterfaceAddresses exception";
                    freeifaddrs(results);
                    return false;
                }
                freeifaddrs(results);
    ...
    bool Address::operator<(const Address& rhs) const {
        ...
        int rt = ::memcmp(this->getAddr(), rhs.getAddr(), min_len);
        if (rt < 0) {
            return true;
        } else if (rt > 0) {
            return false;
    ...
    IPAddress::ptr IPAddress::Create(const char* address, uint16_t port) {
        ...
        if(error) {
            SATURN_LOG_DEBUG(g_logger) << "IPAddress::Create(" << address
                << ", " << port << ") error=" << error
    ...
    IPv4Address::ptr IPv4Address::Create(const char* address, uint16_t port) {
        ...
        if(result <= 0) {
            SATURN_LOG_DEBUG(g_logger) << "IPv4Address::Create(" << address << ", "
    ...
    const sockaddr* IPv4Address::getAddr() const {return reinterpret_cast<const sockaddr*>(&m_addr);}
    
    sockaddr* IPv4Address::getAddr() {return reinterpret_cast<sockaddr*>(&m_addr);}
    ...
    IPAddress::ptr IPv4Address::broadcastAddress(uint32_t prefix_len) {
        if (prefix_len > 32)
            return nullptr;
        sockaddr_in braddr {m_addr};
        braddr.sin_addr.s_addr |= byteswapOnLittleEndian(
            CreateMask<uint32_t>(prefix_len));
        return IPv4Address::ptr(new IPv4Address(braddr));
    }
    ...
    IPv6Address::ptr IPv6Address::Create(const char* address, uint16_t port) {
        ...
        if(result <= 0) {
            SATURN_LOG_DEBUG(g_logger) << "IPv6Address::Create(" << address << ", "
                    << port << ") rt=" << result << " errno=" << errno
                    << " errstr=" << strerror(errno);
            return nullptr;
        }
    ...
    const sockaddr* IPv6Address::getAddr() const {return reinterpret_cast<const sockaddr*>(&m_addr);}
    sockaddr* IPv6Address::getAddr() {return reinterpret_cast<sockaddr*>(&m_addr);} 
    ...
    IPAddress::ptr IPv6Address::broadcastAddress(uint32_t prefix_len)  {
        sockaddr_in6 braddr(m_addr);
        braddr.sin6_addr.s6_addr[prefix_len / 8] |=
            CreateMask<uint8_t>(prefix_len % 8);
        for(int i = prefix_len / 8 + 1; i < 16; ++i) {
            braddr.sin6_addr.s6_addr[i] = 0xff;
        }
        return IPv6Address::ptr(new IPv6Address(braddr));
    }
    ...
    UnixAddress::UnixAddress() {
        ...
        m_length = offsetof(sockaddr_un, sun_path) + sizeof(((sockaddr_un*)0)->sun_path) - 1;
    }
    ...
    const sockaddr* UnixAddress::getAddr() const {return reinterpret_cast<const sockaddr*>(&m_addr);}
    
    sockaddr* UnixAddress::getAddr() {return reinterpret_cast<sockaddr*>(&m_addr);}    
    ...
    const sockaddr* UnknownAddress::getAddr() const {return reinterpret_cast<const sockaddr*>(&m_addr);}
    
    sockaddr* UnknownAddress::getAddr() {return reinterpret_cast<sockaddr*>(&m_addr);}
